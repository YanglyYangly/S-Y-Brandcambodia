...
function megashop_is_support_expired($slug){

  $date = date('m/d/Y h:i:s a', time());

  if(strtotime($date) > strtotime(get_option( $slug . '_tt_supported_until', '' ))){
    return true;
  }
}

/* Check if theme is enabled */
function megashop_is_theme_enabled(){
  $slug = basename( get_template_directory() );
  $purchase_code = sanitize_text_field( get_option( $slug . '_tt_purchase_code', '' ) );
  if($purchase_code){
    return true;
  }
}
...
/* Automagical updates */
function tt_wupdates_check( $transient ) {
...
add_filter( 'pre_set_site_transient_update_themes', 'tt_wupdates_check' );

/* Only allow theme updates with a valid Envato purchase code */
function tt_wupdates_add_purchase_code_field( $themes ) {
  $output = '';
  // First get the theme directory name (the theme slug - unique)
  $slug = basename( get_template_directory() );
  if ( ! is_multisite() && isset( $themes[ $slug ] ) ) {
    ...
    $errors = get_option( strtolower( $slug ) . '_tt_errors', array() );
    delete_option( strtolower( $slug ) . '_tt_errors' ); //delete existing errors as we will handle them next

    //check if we have a purchase code saved already
    $purchase_code = sanitize_text_field( get_option( strtolower( $slug ) . '_tt_purchase_code', '' ) );
    ...
    if ( empty( $purchase_code ) && ! empty( $themes[ $slug ]['hasUpdate'] ) ) {
      $output .= '<div class="notice notice-error notice-alt notice-large">' . __( 'A <strong>valid purchase code</strong> is required for automatic updates.', 'megashop' ) . '</div>';
    }
    ...
    if ( ! empty( $purchase_code ) ) {
      if ( ! empty( $errors ) ) {
        ...
        $output .= '<div class="notice notice-warning notice-alt"><p>' . esc_html__( 'Purchase code not updated. We will keep the existing one.', 'megashop' ) . '</p></div>';
      } else {
        //this means a valid purchase code is present and no errors were found
        $output .= '<div class="notice notice-success notice-alt notice-large">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy one-click automatic updates.', 'megashop' ) . '</div>';
      }
    }
    $purchase_code_key = esc_attr( strtolower( str_replace( array(' ', '.'), '_', $slug ) ) ) . '_tt_purchase_code';
    $output .= '<form class="wupdates_purchase_code" action="" method="post">' .
      ...
      '<input type="text" id="' . $purchase_code_key . '" name="' . $purchase_code_key . '"
              value="' . esc_attr( $purchase_code ) . '" placeholder="' . esc_html__( 'Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )', 'megashop' ) . '" style="width:100%"/>' .
      '<p>' . __( 'Enter your purchase code and <strong>hit return/enter</strong>.', 'megashop' ) . '</p>' .
      '<p class="theme-description">' .
        __( 'Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.', 'megashop' ) .
        '</p>
      </form>';
  }
...
add_filter( 'wp_prepare_themes_for_js' ,'tt_wupdates_add_purchase_code_field' );
...
function wupdates_ms_theme_list_purchase_code_field_JQ9eJ( $theme, $r ) {
  ...
  //in case there is an update available, tell the user that it needs a valid purchase code
  if ( empty( $purchase_code ) ) {
    $output .=  '<p>' . __( 'A <strong>valid purchase code</strong> is required for automatic updates.', 'megashop' ) . '</p>';
  }
  //output errors and notifications
  if ( ! empty( $errors ) ) {
    foreach ( $errors as $key => $error ) {
  ...
  if ( ! empty( $purchase_code ) ) {
    if ( ! empty( $errors ) ) {
      //since there is already a purchase code present - notify the user
      $output .= '<p>' . esc_html__( 'Purchase code not updated. We will keep the existing one.', 'megashop' ) . '</p>';
    } else {
      //this means a valid purchase code is present and no errors were found
      $output .= '<p><span class="notice notice-success notice-alt">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy one-click automatic updates.', 'megashop' ) . '</span></p>';
    }
  }
  $purchase_code_key = esc_attr( strtolower( str_replace( array(' ', '.'), '_', $slug ) ) ) . '_tt_purchase_code';
  $output .= '<form class="wupdates_purchase_code" action="" method="post">' .
    '<input type="hidden" name="wupdates_pc_theme" value="' . esc_attr( $slug ) . '" />' .
    '<input type="text" id="' . $purchase_code_key . '" name="' . $purchase_code_key . '"
            value="' . esc_attr( $purchase_code ) . '" placeholder="' . esc_html__( 'Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )', 'megashop' ) . '"/>' . ' ' .
    __( 'Enter your purchase code and <strong>hit return/enter</strong>.', 'megashop' ) . ' ' .
    __( 'Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.', 'megashop' ) .
    '</form>';
...
function tt_wupdates_purchase_code_needed_notice() {
  ...
  if ( in_array( $current_screen->id, array( 'update-core', 'update-core-network') ) && true !== tt_wupdates_prevalidate_purchase_code( $purchase_code ) ) {
    $output .= '<div class="updated"><p>' . sprintf( __( '<a href="%s">Please enter your purchase code</a> to activate megashop and get automatic updates.', 'megashop' ), network_admin_url( 'admin.php?page=megashop-panel' ) ) . '</p></div>';
  }

  echo do_shortcode($output);
}
add_action( 'admin_notices', 'tt_wupdates_purchase_code_needed_notice' );
add_action( 'network_admin_notices', 'tt_wupdates_purchase_code_needed_notice' );

function tt_wupdates_process_purchase_code() {
  ...
  if ( ! empty( $_POST['wupdates_pc_theme'] ) ) {
    ...
    if ( ! empty( $_POST[ $purchase_code_key ] ) ) {
      ...
      if ( true !== tt_wupdates_prevalidate_purchase_code( $purchase_code ) ) {
        $purchase_code = false;
      }
    }
    if ( ! empty( $purchase_code ) ) {
      ...
      if ( is_wp_error( $raw_response ) || 200 != wp_remote_retrieve_response_code( $raw_response ) ) {
        $errors[] = __( 'We are sorry but we couldn\'t connect to the verification server. Please try again later.', 'megashop' ) . '<span class="hidden">' . print_r( $raw_response, true ) . '</span>';
      } else {
        ...
        if ( ! empty( $response ) ) {
          ...
          if ( isset( $response['purchase_code'] ) && 'valid' == $response['purchase_code'] ) {
            ...
            update_option( strtolower( $slug ) . '_tt_purchase_code', $purchase_code );

            // Sold at
            update_option( strtolower( $slug ) . '_tt_sold_at', $response['raw_response']['sold_at'] );

            // Supported until
            update_option( strtolower( $slug ) . '_tt_supported_until', $response['raw_response']['supported_until'] );

            // Buyer
            update_option( strtolower( $slug ) . '_tt_buyer', $response['raw_response']['buyer'] );
            ...
            if ( isset( $response['reason'] ) && ! empty( $response['reason'] ) && 'out_of_support' == $response['reason'] ) {
              $errors[] = esc_html__( 'Your purchase\'s support period has ended. Please extend it to receive automatic updates.', 'megashop' );
            } else {
              $errors[] = esc_html__( 'Could not find a sale with this purchase code. Please double check.', 'megashop' );
            }
          }
        }
      }
      ...
      $errors[] = esc_html__( 'Please enter a valid purchase code. Make sure to get all the characters.', 'megashop' );
    }

    if ( count( $errors ) > 0 ) {
      //if we do have errors, save them in the database so we can display them to the user
      update_option( strtolower( $slug ) . '_tt_errors', $errors );
    } else {
      //since there are no errors, delete the option
      delete_option( strtolower( $slug ) . '_tt_errors' );
    }
...
add_action( 'admin_init', 'tt_wupdates_process_purchase_code' );

function tt_wupdates_send_purchase_code( $optional_data, $slug ) {
...
add_filter( 'wupdates_call_data_request', 'tt_wupdates_send_purchase_code', 10, 2 );

function tt_wupdates_prevalidate_purchase_code( $purchase_code ) {
