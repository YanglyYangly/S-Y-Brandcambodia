...
add_action('save_post', 'tt_post_format_media');
//Flushing transients
 ...
 * @param  string $format
 */
if (!function_exists('tt_get_post_format_media')) {

    function tt_get_post_format_media($format = null) {
        //Helper variables
        if (empty($format)) {
        ...
        //Output
        return tt_post_format_media(get_the_ID(), $format);
    }
 ...
 * @param  int $post_id
 */
if (!function_exists('tt_post_format_media')) {

    function tt_post_format_media($post_id = null, $format = null) {
        //Requirements check
        if (empty($post_id)) {
            $post_id = get_the_ID();
        }
        ...
        $meta_name = apply_filters('tt_post_format_media_meta_name', 'post_format_media');

        $supported_formats = apply_filters('tt_post_format_media_formats', array('audio', 'gallery', 'image', 'video'));
        ...
        $output = apply_filters('tt_post_format_media_output_pre', $output, $post_id, $format);

        //Force refresh (regenerate and resave) the post media meta field
        if (
        //when forced
                apply_filters('tt_post_format_media_force_refresh', false, $post_id, $format)
                //when no media saved
                || empty($output)
        ...
        if ($output) {
            return apply_filters('tt_post_format_media_output', $output, $post_id, $format);
        }
        ...
        switch ($format) {
            ...
            case 'video':
                ...
                $output = tt_get_post_media_audio_video($post_id);

                break;
            case 'gallery':

                $output = tt_get_post_media_gallery($post_id);

                break;
            case 'image':

                $output = tt_get_post_media_image($post_id);
        ...
        //Filter the output
        $output = apply_filters('tt_post_format_media_output', $output, $post_id, $format);

        //Save the post media meta field
        update_post_meta($post_id, '_' . $meta_name, $output);

        //Custom action hook
        do_action('tt_post_format_media', $output, $post_id, $format, $meta_name);

        //Output
        return $output;
    }

} // /tt_post_format_media
 ...
 */
if (!function_exists('tt_get_post_media_audio_video')) {

    function tt_get_post_media_audio_video($post_id) {
        //Requirements check
        if (empty($post_id)) {
            return;
        }
 ...
 */
if (!function_exists('tt_get_post_media_gallery')) {

    function tt_get_post_media_gallery($post_id) {
        //Requirements check
        if (empty($post_id)) {
            return;
        }
        ...
        //Output
        return apply_filters('tt_get_post_media_gallery_output', $output, $post_id);
    }
 ...
 */
if (!function_exists('tt_get_post_media_image')) {

    function tt_get_post_media_image($post_id) {
        //Requirements check
        if (
                empty($post_id)
        ...
        if ($image_id = tt_get_image_id_from_url($output)) {
            $output = $image_id;
        }
 ...
 * @param  string $url
 */
if (!function_exists('tt_get_image_id_from_url')) {

    function tt_get_image_id_from_url($url) {
        //Helper variables
        global $wpdb;
 ...
 */
if (!function_exists('tt_image_ids_transient_flusher')) {

    function tt_image_ids_transient_flusher() {
