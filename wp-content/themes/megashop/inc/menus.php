...
/**
 * Class TT_Walker_Mega_Menu
 *
 * Walker class for mega menu
 */
class TT_Walker_Mega_Menu extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_mega = TT_parse_args( $item_mega, TT_get_mega_menu_setting_default() );
		...
		if ( $item_mega['mega'] && ! $depth ) {
			...
			if ( $item_mega['background']['image'] ) {
				$classes[] = 'menu-item-has-background';
                    ...
                    $classes[] = 'menu-item-flyout';
                }
                $parent_settings = array_filter( (array) get_post_meta( $item->menu_item_parent, '_menu_item_mega', true ) );
                $parent_settings = TT_parse_args( $parent_settings, TT_get_mega_menu_setting_default() );
                $total_columns = $parent_settings['megaColumn'];
                
                
		if ( 1 == $depth && $this->in_mega ) {
                    
			$classes[] = 'mega-sub-menu ' . $this->get_css_column( $item_mega['width'] );                        
                        $span = $item_mega['menuColumn'];
                        if($total_columns == 2 && $span == 2){
                            $span = 1;
                        }
                        ...
                        if ( ! isset( $this->parents[ $item->menu_item_parent ] ) ) {
                            $this-> parents[ $item->menu_item_parent ] = $column_count;
                        } else {
                            $this->parents[ $item->menu_item_parent ] = $this-> parents[ $item->menu_item_parent ] + $column_count;

                            if ( $this->parents[ $item->menu_item_parent ] > $total_columns ) {
                               $this-> parents[ $item->menu_item_parent ] = $column_count ;
                                $classes[] = 'menu-clear';
                            }
                        }
                        //print_r($parents);
			if ( $item_mega['disable_link'] == "1" ) {
				$classes[] = 'link-disabled';
			}

			if ( $item_mega['hide_text'] == "1" ) {
				$classes[] = 'menu-item-title-hidden';
			}

			if ( $item_mega['border']['left'] == "1" ) {
		...
		if ( $this->in_mega && $depth == 1 && $item_mega['disable_link'] == "1" ) {
			$link_open = '<span>';
		} else {
			$link_open = '<a' . $attributes . ' data-depth= ' .$depth . '>';
		}
		// Adds icon
		if ( $item_mega['icon'] && empty( $item_mega['iconimage'] )) {
			$icon = '<i class="' . esc_attr( $item_mega['icon'] ) . '"></i>';
		} else {
			$icon = '';
		}
                if ( $item_mega['iconimage'] ) {
			$icon = '<img src="' . esc_attr( $item_mega['iconimage'] ) . '" style="width:30px;height:30px;">';
		} else {
			$icon = '';
		}
		...
		if ( 1 <= $depth && ! empty( $item_mega['content'] ) ) {
			$item_output .= '<div class="menu-item-content">' . do_shortcode( wp_specialchars_decode($item_mega['content']) ) . '</div>';
		}
 ...
 * @return array
 */
function TT_nav_menu_args( $args ) {
	if ( ! in_array( $args['theme_location'], array( 'top_menu', 'footer', 'socials' ) ) ) {
		$args['walker'] = new TT_Walker_Mega_Menu;
	}
...
add_filter( 'wp_nav_menu_args', 'TT_nav_menu_args' );