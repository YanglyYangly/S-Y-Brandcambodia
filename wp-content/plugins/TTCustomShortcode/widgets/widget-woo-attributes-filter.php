 ...
 * @extends  WC_Widget
 */
class TT_Widget_Attributes_Filter extends WC_Widget {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce tt__attributes_filter widget_layered_nav';
		$this->widget_description = esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'megashop' );
		$this->widget_id          = 'tt__attributes_filter';
		$this->widget_name        = esc_html__( 'TT Attributes Filter', 'megashop' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title'      => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by', 'megashop' ),
				'label' => esc_html__( 'Title', 'megashop' )
			),
			'attribute'  => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'megashop' ),
				'options' => $attribute_array
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'megashop' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'megashop' ),
					'or'  => esc_html__( 'OR', 'megashop' )
				)
			),
		);
	}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $show_color ) {
		...
		foreach ( $terms as $term ) {
			...
			echo apply_filters( 'tt__color_filter_nav_count', '<span class="count">' . absint( $count ) . '</span>', $count, $term );
...
function tt__register_widgets() {
	if ( class_exists( 'WC_Widget' ) ) {
		register_widget( 'TT_Widget_Attributes_Filter' );
	}
}
add_action( 'widgets_init', 'tt__register_widgets' );