 ...
 * @see   WP_Widget
 */
class TT_Price_Filter_List_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops  = array(
			'classname'   => 'widget_text tt-price-filter-list',
			'description' => esc_html__( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'megashop' ),
		);
		$control_ops = array( 'width' => 400, 'height' => 350 );
		parent::__construct( 'tt-price-filter-list', esc_html__( 'TT - Price Filter List', 'megashop' ), $widget_ops, $control_ops );
	}
	...
	public function form( $instance ) {
		...
		?>
		<p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title:', 'megashop' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>

		<p><label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_html_e( 'Content:', 'megashop' ); ?></label>
		...
		<p><?php esc_html_e( 'Enter the price by format: min_price|max_price. Divide price list with linebreaks (Enter). Example:', 'megashop' ); ?></p>
		<p><?php esc_html_e( '100|200', 'megashop' ); ?><br><?php esc_html_e( '200|300', 'megashop' ); ?>
			<br><?php esc_html_e( '300|400', 'megashop' ); ?></p>
		<?php
	}
}

function tt_Price_Filter_List_Widget() {
	if ( class_exists( 'WC_Widget' ) ) {
		register_widget( 'TT_Price_Filter_List_Widget' );
	}
}
add_action( 'widgets_init', 'tt_Price_Filter_List_Widget' );