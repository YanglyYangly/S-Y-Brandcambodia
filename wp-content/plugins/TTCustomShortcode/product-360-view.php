 ...
 * ------------------------------------------------------------------------------------------------
 */
if( ! function_exists( 'tt_product_360_view_meta' ) ) {
	add_action( 'add_meta_boxes', 'tt_product_360_view_meta', 50 );
	function tt_product_360_view_meta() {
		add_meta_box( 'woocommerce-product-360-images', __( 'Product 360 View Gallery (optional)', 'megashop' ), 'tt_360_metabox_output', 'product', 'side', 'low' );
	}
}
if( ! function_exists( 'tt_360_metabox_output' ) ) {
	function tt_360_metabox_output( $post ) {
		?>
					...
					if ( ! empty( $attachments ) ) {
						foreach ( $attachments as $attachment_id ) {
							...
							echo '<li class="image" data-attachment_id="' . esc_attr( $attachment_id ) . '">
								' . $attachment . '
								<ul class="actions">
									<li><a href="#" class="delete tips" data-tip="' . esc_attr__( 'Delete image', 'megashop' ) . '">' . __( 'Delete', 'megashop' ) . '</a></li>
								</ul>
							</li>';
		...
		<p class="add_product_360_images hide-if-no-js">
			<a href="#" data-choose="<?php esc_attr_e( 'Add Images to Product 360 view Gallery', 'megashop' ); ?>" data-update="<?php esc_attr_e( 'Add to gallery', 'megashop' ); ?>" data-delete="<?php esc_attr_e( 'Delete image', 'megashop' ); ?>" data-text="<?php esc_attr_e( 'Delete', 'megashop' ); ?>"><?php _e( 'Add product 360 view gallery images', 'megashop' ); ?></a>
		</p>
		<?php
 ...
 * ------------------------------------------------------------------------------------------------
 */
if( ! function_exists( 'tt_proccess_360_view_metabox' ) ) {
	add_action( 'woocommerce_process_product_meta', 'tt_proccess_360_view_metabox', 50, 2 );
	function tt_proccess_360_view_metabox( $post_id, $post ) {
 ...
 * ------------------------------------------------------------------------------------------------
 */
if( ! function_exists( 'tt_get_360_gallery_attachment_ids' ) ) {
	function tt_get_360_gallery_attachment_ids() {
		global $post;
